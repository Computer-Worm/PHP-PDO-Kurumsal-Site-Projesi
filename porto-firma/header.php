<?php 
	...
	include 'admin/production/fonki.php';
    ...
    // {
    //   $aranan = $_POST['aranan'];
    //   $iceriksor = $db->prepare("SELECT * FROM icerik WHERE icerik_ad LIKE '%$aranan%' AND icerik_durum=1 ORDER BY icerik_ad ASC LIMIT 25");
    //   $iceriksor->execute();
    //   $say = $iceriksor->rowCount();
    // }
    // else
    // {
    //   $iceriksor = $db->prepare("SELECT * FROM icerik WHERE icerik_durum=1 ORDER BY icerik_ad ASC LIMIT 25");   
    //   $iceriksor->execute(array('menu' => 0));
    //   $say = $iceriksor->rowCount();
    // }
										...
										?>>
											<a href="<?php 
												if (strlen($menucek['menu_url']) > 0)
												{
													...
													sayfa-<?=fonki($menucek['menu_ad']).'-'.$menucek['menu_id'] ?>
												<?php }
												?>">
													...
													?>
														<li>
															<a href="<?php 
																...
																else if (strlen($altmenucek['menu_url']) == 0)	{ ?>
																	sayfa-<?=fonki($altmenucek['menu_ad']).'-'.$altmenucek['menu_id'] ?>
																<?php }
																?>">
